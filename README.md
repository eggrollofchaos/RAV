...
Current app version: `v0.2.39-wrapper-dispatch-runtime-contract`
Spot runner lineage version: `gcp-spot-runner v0.6.31-wrapper-dispatch-runtime-contract`
...
18. `gcp/` + `scripts/rav-gcp.sh` + `scripts/gcp_*.sh` (GCP spot-runner adapter; unified CLI + thin wrappers)
...
2. `scripts/rav-gcp.sh` is the canonical operator entrypoint; `scripts/gcp_*.sh` remain thin wrappers for compatibility and direct calls.
3. `gcp_build_image.sh` routes build execution via `spotctl build --profile rav`, including runner-owned staged-source fallback.
4. Wrapper submits default to `--skip-build`, so build image first with `./scripts/rav-gcp.sh build` (or explicit `scripts/gcp_build_image.sh`).
5. `gcp_monitor.sh` is the thin monitor wrapper over `spotctl monitor --profile rav`.
6. Submit wrappers run `scripts/gcp_train_with_checkpoint_sync.sh` for both primary and POC tracks.
7. During training, wrapper syncs `checkpoints/last.pt`, `checkpoints/best.pt`, and metrics to GCS every `SYNC_INTERVAL_SEC`.
8. On restart with the same `RUN_ID`, wrapper auto-downloads `last.pt` and resumes via `--resume-checkpoint`.
9. Wrapper copies `outputs/...` into `/app/results/...` at run end for runner artifact upload.
10. If Cloud Build fails with `COPY ... gcp/state_transitions.json`, verify `gcloud meta list-files-for-upload` includes `gcp/state_transitions.json`, then retry `./scripts/rav-gcp.sh build`.
11. If staged-source fallback fails with `storage.objects.get` 403, grant bucket read (`roles/storage.objectViewer`) to the service account named in the error.
12. On COS with persistent disk enabled, use a writable host mount path:
